...
class DistinctSubstring {
	...
	public static void main(String[] args) {
		...
		while(tc-- > 0) {
			...
			for(; cnt/2 < n; stp++, cnt<<=1) {
				...
				Arrays.sort(L);
				for(int i = 0; i < n; i++) {
					p[stp][L[i].p] = (i > 0 && L[i].l == L[i-1].l && L[i].r == L[i-1].r) ? p[stp][L[i-1].p] : i;
				}
			}
			int[] a  = new int[n];
			for(int i = 0; i < n; i++) {
			...
			int t = ((n)*(n+1))/2;
			// out.println("Total = " + t + " Repetition = "  + sum + " Result = " + (t-sum));
			out.println((t-sum));
	...
	static class Entry implements Comparable<Entry>{
		...
		public int compareTo(Entry e) {
			return this.l == e.l ? this.r-e.r : this.l-e.l;
		}
