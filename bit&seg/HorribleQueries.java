...
class HorribleQueries {
	...
	public static void main(String[] args) {
		...
		int tc = in.nextInt();
		while(tc-- > 0) {
			ft1 = new long[in.nextInt()+1];
			ft2 = new long[ft1.length];
			int c = in.nextInt();
			while(c-- > 0) {
				int t = in.nextInt();
				int i = in.nextInt();
	...
	static long query(int j) {
		return query(ft1, j)*j - query(ft2, j);
	}

	static long query(long[] f, int i) {
		long s = 0;
		while(i > 0) {
			s+= f[i];
			i-= (i&-i);
	...
	static void update(int i, int j, long v) {
		update(ft1, i, v);
		update(ft1, j+1, -v);
		update(ft2, i, v*(i-1));
		update(ft2, j+1, -v*j);
	...
	static void update(long[] f, int i, long v) {
		while(i < f.length) {
			f[i] += v;
			i += (i&-i);
		}
