...
class MCoins {
	public static void main(String[] args) {
		...
		//if value = 1 then pos is loosing pos for the first player and 
		//if value = 2 is then pos is winning pos
		lookup[0] = 1; lookup[1] = 2; 
		if(k <= max)
			lookup[k] = 2; 
		if(l <= max)
			lookup[l] = 2;
		...
		for(int j = 2; j <= max; j++) {
			if(lookup[j-1] == 1 || (j >= l ? lookup[j-l] == 1 : false) || (j >= k ? lookup[j-k] == 1 : false))
				lookup[j] = 2;
			else
				lookup[j] = 1;
		}
