...
class PiggyBank {
	...
	public static void main(String[] args) 	throws Exception {
		...
		int tc = in.nextInt();
		String result = "The minimum amount of money in the piggy-bank is %d.";
		while(tc-- > 0) {
			 int w1 = in.nextInt();
			 int w2 = in.nextInt();
			 int w = w2-w1;
		 	...
		 	//find(w);
		 	if(lookup[w] == Integer.MAX_VALUE) {
		 		out.println("This is impossible.");
		 	} else {
		 		out.println(String.format(result, lookup[w]));
		 	}
			 // for(int i = 1; i < lookup.length; i++) {
			 // 	out.println(i + " " + lookup[i]);
			 // }
		}
	...
	static int find(int w) {
		...
		if(lookup[w] == 0) {
			//System.out.println("weight " + w + " value ");
			int min = Integer.MAX_VALUE;
