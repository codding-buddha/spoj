...
class CrossCountry {
	public static void main(String[] args) {
		...
		int tc = in.nextInt();
		while(tc-- > 0) {
			List<Integer> agnes = new ArrayList<Integer>();
			int n;
			...
			while((n = in.nextInt()) != 0) {
				int v = 0;
				List<Integer> tom = new ArrayList<Integer>();
				tom.add(n);
				while((v=in.nextInt()) != 0)
					tom.add(v);

				max = Math.max(max, lcs(agnes, tom));
			}	
	...
	static int lcs(List<Integer> a, List<Integer> b) {
		int[][] lookup = new int[a.size()+1][b.size()+1];
		for(int i = 0, l1 = a.size(); i <= l1; i++) {
			for(int j = 0, l2 = b.size(); j <= l2; j++) {
				if(i == 0 || j == 0) {
					lookup[i][j] = 0;
