...
class MartianMining {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			for(int i = 0; i < n; i++) {
				for(int j = 0; j < m; j++) {
					bo[i][j] = in.nextInt();
					bo[i][j] += i > 0 ? bo[i-1][j] : 0;
				}
			}

			lookup = new long[n][m];
			out.println(maxMineral(n-1, m-1));
		}
	...
	static long maxMineral(int r, int c) {
		if(r < 0 || c < 0)
			return 0;

		if(lookup[r][c] == 0) {
			if(r == 0 && c == 0){
				lookup[r][c] = Math.max(ye[0][0], bo[0][0]);
			} else {
				lookup[r][c] = Math.max(ye[r][c] + maxMineral(r-1, c), bo[r][c] + maxMineral(r, c-1));
			}
		}
