...
class BugLife {
	...
	public static void main(String[] args) throws Exception{
		...
		while(tc-- > 0) {
			...
			out.println(String.format("Scenario #%d:", sc++));
			out.println(possible ? "No suspicious bugs found!" : "Suspicious bugs found!");
		}
		out.flush();
		out.close();
	}

	static boolean assign(int indx, int g) {
		if(visited[indx] && gender[indx] != g)
			return false;
		...
		List<Integer> node = graph[indx];
		for(int j = 0, len = node.size(); j < len; j++) {
			int ni = node.get(j);
			if(!visited[ni]) {
				boolean v = assign(ni, g == 1 ? 2 : 1);
				if(!v)
					return v;
			}
			else {
