...
class Trigraph {
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			in.nextInt();
			lookup[1][2] = in.nextInt();
			lookup[1][3] = lookup[1][2] + in.nextInt();

			for (int i = 2; i <= n; i++) {
				for (int j = 1; j <= 3; j++) {
					lookup[i][j] = Math.min(Math.min(lookup[i-1][j], lookup[i-1][j-1]),
											Math.min(lookup[i][j-1], lookup[i-1][j+1]))
									+ in.nextInt();
				}
			}

			out.println(String.format("%d. %d", tc++, lookup[n][2]));
