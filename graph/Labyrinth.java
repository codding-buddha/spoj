...
class Labyrinth {
	...
	public static void main(String[] args) {
		...
		while(tc-- > 0) {
			...
			out.println(String.format("Maximum rope length is %d.", mx));
	...
	static Context dfs(Context ctxt) {
		boolean[][] visited = new boolean[r][c];
		Stack<Context> s = new Stack<Context>();
		s.push(ctxt);
		int mx = 0;
		...
		while(s.size() > 0) {
			Context ct = s.pop();
			visited[ct.r][ct.c] = true;
			int ri = ct.r, ci = ct.c;
			for(int d = 0; d < 4; d++) {
				int xd = x[d] + ri, yd = y[d] + ci;
				if(xd >= 0 && xd  < r && yd < c && yd >= 0 && l[xd][yd] != -1 && !visited[xd][yd]){
					if((ct.d + 1) > mx){
						// leaf = ct;
						mx = ct.d + 1;
					}
					
					mxContext = new Context(xd, yd, ct.d + 1);
					s.push(mxContext);
				}
			}
		}
	...
	static class Context {
		...
		public Context(int row, int col, int data) {
			r = row;
			c = col;
			d = data;
		}
