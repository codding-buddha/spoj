...
class PenneyGame {
	public static void main(String[] args) {
		...
		int ds = in.nextInt();
		String[] sequences = new String[] {"TTT", "TTH", "THT", "THH", "HTT", "HTH", "HHT", "HHH"};
		while(ds-- > 0) {
			int c = in.nextInt();
			String seq = in.nextString();
			out.print(c + " ");
			for(int i = 0; i < sequences.length; i++)
				out.print(occurances(seq, sequences[i]) + " ");
			out.println();
		}
	...
	static int occurances(String s, String p) {
		int c = 0;

		for(int i = 0, l = s.length() - p.length(); i <= l; i++) {
			for(int j = 0, pl = p.length(); j < pl; j++) {
